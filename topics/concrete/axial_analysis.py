...
try:
    # PchipInterpolator is monotonic (prevents overshoot/fake peaks)
...
def plot_load_deformation(N1, N2, trans_type):
    """
    Plots the N-Delta curve showing First Peak (Unconfined) and Second Peak (Confined).
    Handles the 'dip' and 'plateau' logic.
    """
    ...
    if trans_type == "Spiral":
        if N2 > N1:
            # Case: Successful Confinement (Dip then Rise)
            # x points represent strain/shortening stages
            x = np.array([0, 1.0, 2.0, 3.5, 5.5, 6.5]) 
            y = np.array([0, N1,  0.85*N1, N2, N2, N2*0.9]) # Plateau then slight drop
            ...
            # Case: Unconfined Governs (Spiral too weak)
            ...
            ax.annotate('Spiral too weak\n(No 2nd Peak)', xy=(3.5, N2), xytext=(3.5, N2+N2*0.15),
                        arrowprops=dict(facecolor=text_color, arrowstyle='->'), ha='center', color=text_color)

    else: # Ties
        # Standard Brittle Failure
    ...
    if HAS_SCIPY:
        try:
            # Use PchipInterpolator for Monotonicity (Fixes overshoot)
...
def app():
    ...
    with col_input:
        ...
        # Spiral Inputs (Only for Analysis of Capacity)
        spiral_dia = 8.0
        spiral_spacing = 80.0
    ...
    with col_viz:
        ...
        st.caption(f"**Section Data:**")
        st.caption(f"$A_g = {Ag:,.0f}$ mm²")
        st.caption(f"$A_{{st}} = {Ast:,.0f}$ mm² ($\\rho = {(Ast/Ag)*100:.2f}\\%$)")
    ...
    if st.button("Analyze Capacity", type="primary"):
        ...
        # --- TS 500 ANALYSIS (Detailed) ---
            ...
            # --- PEAK 1: UNCONFINED (SHELL) ---
            st.markdown("### Peak 1: Unconfined Capacity ($N_{or}$)")
            st.write("Strength provided by the gross section (concrete shell + core) assuming no confinement.")
            
            # Formula: Nor = 0.85 fcd (Ag - Ast) + Ast fyd
            ...
            st.latex(fr"N_{{or}} = {term_conc/1000:.0f} + {term_steel/1000:.0f} = \mathbf{{{Nor1/1000:.0f} \text{{ kN}}}}")
            
            graph_N1 = Nor1 / 1000
            graph_N2 = 0
            
            # --- PEAK 2: CONFINED (CORE) ---
            if trans_type == "Spiral":
                st.markdown("---")
                st.markdown("### Peak 2: Confined Capacity ($N_{or2}$)")
                st.write("Strength after shell spalling. Strength comes from Confined Core + Steel.")
                ...
                # 2. Spiral Ratio (Using Centerline Diameter)
                ...
                # 3. Check Minimum Ratio
                rho_min_calc = 0.45 * (fc/fy) * ((Ag/Ack)-1)
                rho_min_abs = 0.12 * (fc/fy)
                rho_min_req = max(rho_min_calc, rho_min_abs)
                
                if rho_s < rho_min_req:
                    st.error(f"⚠️ $\\rho_s$ ({rho_s:.4f}) < Min Required ({rho_min_req:.4f}). Second peak will not develop!")
                else:
                    st.success(f"$\\rho_s$ ({rho_s:.4f}) > Min Required ({rho_min_req:.4f})")

                # 4. Confined Strength
                # f_ccd = (0.85 fck + 2 rho_s fyk) / 1.5
                ...
                # 5. Capacity
                # Nor2 = f_ccd * Ack + Ast * fyd  (Note: Notes usually use Gross Ack, sometime Net)
                # Let's use Example 1d method: Confined Strength * Core Area + Steel Yield
                term_core = f_ccd * Ack
                term_steel_2 = Ast * fyd
                Nor2 = term_core + term_steel_2
                
                st.latex(r"N_{or2} = f_{ccd} A_{ck} + A_{st} f_{yd}")
                st.latex(fr"N_{{or2}} = {f_ccd:.2f}({Ack:.0f}) + {Ast:.0f}({fyd:.2f})")
                st.latex(fr"N_{{or2}} = {term_core/1000:.0f} + {term_steel_2/1000:.0f} = \mathbf{{{Nor2/1000:.0f} \text{{ kN}}}}")
                ...
                if delta > 0:
                    ...
                    st.warning(f"⚠️ **Unconfined Peak Governs.** Loss = {delta:.0f} kN. Spiral is insufficient to compensate for shell loss.")
